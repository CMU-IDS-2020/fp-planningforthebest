...
"""
script from: https://github.com/m4qo5/logistic-regression-numpy
"""
class LogisticRegression:
    def __init__(self, learning_rate=0.0001, num_iter=10000, fit_intercept=True, verbose=False):
    ...
    def fit(self, X, y):
        ...
        self._weights = np.matrix(np.zeros(X.shape[1])) #  inicializing our weights vector filled with zeros
        for i in range(self.num_iter):  # implementing Gradient Descent algorithm
            z = np.dot(X, self._weights.T)  #  calculate the product of the weights and predictor matrix
            h = self._sigmoid(z)

            gradient = (np.multiply(X, (h.T - y).T) / y.size).sum(axis = 0).reshape(self._weights.shape)
     

            self._weights -= self.learning_rate * gradient
            
            if ( i % 10000 == 0):
                z = np.dot(X, self._weights.T)
                h = self._sigmoid(z)
    ...
    def predict_proba(self, X):  
        ...
        return self._sigmoid(np.dot(X, self._weights.T))
